...
type ONUInfoPerBoard struct {
	...
	TXPower      string            `json:"tx_power"`      // The transmitting power of the ONU
	Status       string            `json:"status"`        // The current status of the ONU
	Ethernet     ONUEthernetStatus `json:"ethernet"`      // PON-list Ethernet UNI snapshot
}
...
type ONUCustomerInfo struct {
	Ethernet             ONUEthernetStatus `json:"ethernet"`                // live ONU Ethernet UNI observations
	...
	ObservedAt           string            `json:"observed_at,omitempty"`   // collector UTC time of this live read
}
...
// OnuOnlyID is a remnant upstream cache shape (ONU id only). Live collection
// uses OnuID / OnuSerialNumber / ONUInfoPerBoard. Kept because unused Redis
// helpers and their tests still compile against it.
