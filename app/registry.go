...
// the configured OLT set. Configuration changes require a restart.
...
func (reg *OLTRegistry) GetDefault() (*OLTEntry, bool) {
	...
	// Fallback for a failed default initialization.
...
// engine used at startup from the static configuration.
//
...
// reconnection. Organization/user ID changes replace the metadata snapshot without
// reconnection.
func (reg *OLTRegistry) Reconcile(newOLTs []config.OLTRuntimeConfig) {
	...
	for _, o := range newOLTs {
		...
		// Readers keep the entry returned by Get after its lock is released.
		// Publish a new metadata snapshot so concurrent requests can finish
		// reading the old one while the existing connections are reused.
		updated := *existing
		updated.OLT = o
		updated.UserID = o.UserID
		reg.entries[o.ID] = &updated
	}
}
...
// Targets returns poller device descriptors for every registered OLT.
func (reg *OLTRegistry) Targets() []poller.DeviceTarget {
	reg.mu.RLock()
	defer reg.mu.RUnlock()
	out := make([]poller.DeviceTarget, 0, len(reg.entries))
	ids := make([]string, 0, len(reg.entries))
	for id := range reg.entries {
		ids = append(ids, id)
	}
	...
	for _, id := range ids {
		...
		out = append(out, poller.DeviceTarget{
			ID:      entry.OLT.ID,
			Vendor:  reg.cfg.DeviceMeta.Vendor,
			Family:  reg.cfg.DeviceMeta.Family,
			Role:    reg.cfg.DeviceMeta.Role,
			Boards:  entry.OLT.Boards,
			Pons:    entry.BoardPons,
			Collect: entry.UC,
		})
	}
	return out
