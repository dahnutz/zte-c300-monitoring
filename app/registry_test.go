...
func TestOLTRegistry_Reconcile_MetadataSnapshot(t *testing.T) {
	e := testOLTEntry("c320", "10.0.0.1", 1, map[int]int{1: 16})
	repo := &mockSnmpRepo{}
	e.Repo = repo
	reg := testRegistry("c320", e)
	...
	if e.UserID != 1 || e.OLT.UserID != 1 {
		t.Fatal("a reader's existing snapshot must not change during reconciliation")
	}
	if got.Repo != repo {
		t.Fatal("metadata changes must reuse the existing SNMP repository")
	}
	if repo.closed {
...
func TestLoadRoutesWithRegistry_PerOLTValidation(t *testing.T) {
	...
	router := loadRoutesWithRegistry(reg, nil, nil, "", nil)
...
func TestLoadRoutesWithRegistry_TenantIsolation(t *testing.T) {
	...
	router := loadRoutesWithRegistry(reg, nil, users, "", nil)
